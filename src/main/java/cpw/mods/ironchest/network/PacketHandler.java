...
public enum PacketHandler {
    ...
    /**
     * This is only called on the client side - it adds an {@link IronChestMessageHandler} to the client side pipeline,
     * since the only place we expect to <em>handle</em> messages is on the client.
     */
    ...
    /**
     * This class simply handles the {@link IronChestMessage} when it's received at the client side It can contain
     * client only code, because it's only run on the client.
     *
     * @author cpw
     *
     */
    private static class IronChestMessageHandler extends SimpleChannelInboundHandler<IronChestMessage> {
        ...
        protected void channelRead0(ChannelHandlerContext ctx, IronChestMessage msg) throws Exception {
            ...
            if (te instanceof TileEntityIronChest icte) {
                ...
                // ironBarrel.setFacing(msg.facing);
                // ironBarrel.handlePacketData(msg.type, msg.itemStacks);
            }
        }
    }

    /**
     * This is our "message". In fact, {@link FMLIndexedMessageToMessageCodec} can handle many messages on the same
     * channel at once, using a discriminator byte. But for IronChest, we only need the one message, so we have just
     * this.
     *
     * @author cpw
     *
    ...
    /**
     * This is the codec that automatically transforms the {@link FMLProxyPacket} which wraps the client and server
     * custom payload packets into a message we care about.
     *
     * @author cpw
     *
     */
    private class IronChestCodec extends FMLIndexedMessageToMessageCodec<IronChestMessage> {
        ...
        /**
         * We register our discriminator bytes here. We only have the one type, so we only register one.
         */
    ...
    /**
     * This is a utility method called to transform a packet from a custom packet into a "system packet". We're called
     * from {@link TileEntity#getDescriptionPacket()} in this case, but there are others. All network packet methods in
     * minecraft have been adapted to handle {@link FMLProxyPacket} but general purpose objects can't be handled sadly.
     *
     * This method uses the {@link IronChestCodec} to transform a custom packet {@link IronChestMessage} into an
